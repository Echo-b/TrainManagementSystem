#include "HeadFile.h"
Ppassenger* search_realated_Pasinf(PassengerTableType& passenger_table,char * input_realated_info) {
	//参数为乘客数据表和一个输入的参数，可以是紧急联系人或者证件号，姓名等，查找成功返回一个结构体指针数组首地址
	int is_final_number=0;//用于统计匹配人数 
	Ppassenger passenger_point[10] = {NULL};//声明一个结构体指针数组，用于储存查找结果
