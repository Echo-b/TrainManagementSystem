...
Status ticket_read_file(TicketTableType& ticket_table, const char* file_name)
{
	...
	while (fscanf(fp, "%s", ticket_table.trains_ticket[i].order_number) != -1) {
		fscanf(fp,"%4d%2d%2d", &ticket_table.trains_ticket[i].travel_data.year, &ticket_table.trains_ticket[i].travel_data.month, &ticket_table.trains_ticket[i].travel_data.day);
